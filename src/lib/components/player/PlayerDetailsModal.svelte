...
<div class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6">
	...
	<section
		...
		class="relative z-10 flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl border border-(--tertiary)/10 bg-neutral-900 shadow-2xl shadow-black/60"
	>
		<button
			type="button"
			onclick={onClose}
			aria-label="Fechar"
			class="absolute right-4 top-4 z-30 flex h-10 w-10 items-center justify-center rounded-full border border-white/5 bg-neutral-950/80 text-xl text-neutral-300 backdrop-blur-md transition-colors hover:bg-neutral-800 hover:text-white"
		>
			×
		</button>
		...
		>
			<div
				class="pointer-events-none absolute -left-12 top-0 h-32 w-32 rounded-full bg-(--primary) opacity-15 blur-3xl"
			></div>
			...
			<div class="relative min-w-0">
				<div class="flex flex-wrap items-center gap-2">
					<span
						class="text-[10px] font-bold uppercase tracking-[0.2em] text-(--golden)"
					>
					...
					{#if requestErrors.length > 0}
						<span
							class="rounded-md bg-amber-400/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-300"
		...
		<nav
			class="flex shrink-0 gap-1 overflow-x-auto border-b border-(--tertiary)/5 bg-neutral-900 p-2 custom-scrollbar"
			aria-label="Seções do jogador"
		>
			{#each [
				{ value: 'overview', label: 'Visão geral' },
				{ value: 'market', label: 'Valorização' },
				{ value: 'transfers', label: 'Transferências' }
			] as tab (tab.value)}
				<button
					type="button"
					onclick={() => selectTab(tab.value as Tab)}
					class={`shrink-0 rounded-xl px-4 py-2.5 text-xs font-bold transition-colors ${
						activeTab === tab.value
		...
		<div class="min-h-0 flex-1 overflow-y-auto p-4 custom-scrollbar sm:p-6">
			{#if activeTab === 'overview'}
				<div class="space-y-5">
					...
					<div
						class="flex flex-col gap-4 rounded-2xl border border-(--tertiary)/5 bg-neutral-950/35 p-4 md:flex-row md:items-start md:justify-between sm:p-5"
					>
							...
							<div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
								{#each profile.relatives as relative (relative.id)}
									<div class="rounded-xl bg-neutral-900/60 p-4">
										...
										<span class="mt-1 block text-[10px] uppercase tracking-wider text-neutral-500">
						...
						<div class="flex flex-wrap gap-2">
							{#each socialLinks as link, index (link)}
								<a
									...
									class="rounded-xl border border-(--tertiary)/5 bg-neutral-950/40 px-4 py-2 text-xs font-bold text-neutral-300 transition-colors hover:border-(--secondary)/40 hover:text-(--secondary)"
								>
									Rede social {index + 1}
								</a>
							{/each}
